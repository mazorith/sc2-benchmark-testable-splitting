...
class Identity3(torch.nn.Module):
    def __init__(self):
        super().__init__()
        
    def forward(self, x=[1,2,3] ,y=[1,2,3],z=[1,2,3]):
        h = x
        x = y
        x = h
        return x
...
def evaluate(model_wo_ddp, data_loader=None, iou_types=None, device=None, device_ids=None, distributed=None, no_dp_eval=False,
    ...
    if is_server:
        ...
        for sample_batch, targets in metric_logger.log_every(data_loader, log_freq, header):
            ...
            protocall.send_input_data(sample_batch[0])
        ...
        while not end_dataset:
            ...
            if isinstance(input, str):
                ...
                torch.cuda.synchronize()
                model_time = time.time()
                #print(model_wo_ddp.transform)
                #outputs = model_wo_ddp.transform(sample_batch)
                outputs = model(sample_batch)
                #print(outputs)
                #print(outputs[0].tensors, outputs[0].image_sizes)
                #outputs = model_wo_ddp.backbone.body.bottleneck_layer.encoder(outputs)
                model_time = time.time() - model_time
                print(model_time)
                ...
                protocall.send_encoder_data(send_time)
                del outputs

                metric_logger.update(model_time=model_time, data_size=data_size)
...
def main(args):
    ...
    if(is_server):
        ...
        # modules4 = list(student_model.backbone.body.bottleneck_layer.children())[:1]
